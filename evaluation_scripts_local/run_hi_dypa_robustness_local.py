#!/usr/bin/env python
"""
Convenience driver to run all hi_dypa-robustness configurations locally.

This mirrors `slurm_scripts/run_hi_dypa_robustness_hpc.sh` but is designed
for a single workstation. It sequentially invokes
`evaluation_scripts/evaluate_hi_dypa_robustness.py` for all nine schemes
...
EVAL_SCRIPT = os.path.join(
    REPO_ROOT, "evaluation_scripts", "evaluate_hi_dypa_robustness.py"
)
...
def build_common_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run all hi_dypa-robustness configurations locally using the "
            "evaluate_hi_dypa_robustness.py script."
        )
    )
    parser.add_argument(
        "--python-bin",
...
def collect_and_aggregate_summaries(
        ...
        try:
            ...
            if metrics_by_variant:
                ...
                for variant_key, variant_metrics in sorted(metrics_by_variant.items()):
                    row = base_info.copy()
                    row["deletion_percent"] = variant_metrics.get("deletion_percent", "")
                    row["deletion_mode"] = variant_metrics.get("deletion_mode", "")
                    ...
                    for metric_name, metric_value in variant_metrics.items():
                        if metric_name not in ["deletion_percent", "deletion_mode", "num_results"]:
    ...
    # Sort by scheme, then group_bits, then user_bits, then deletion_percent, then deletion_mode
    sort_columns = ["scheme"]
    if "group_bits" in df.columns:
        sort_columns.append("group_bits")
    if "user_bits" in df.columns:
        sort_columns.append("user_bits")
    if "deletion_percent" in df.columns:
        sort_columns.append("deletion_percent")
    if "deletion_mode" in df.columns:
        sort_columns.append("deletion_mode")
...
def collect_and_aggregate_summaries_concise(
    ...
    config_groups = {}
    ratio_field = "deletion_percent"  # For robustness
                ...
                for variant_key, variant_metrics in metrics_by_variant.items():
                    ...
                    if ratio is not None:
                        ...
                        variant_metrics_clean = {
                            ...
                            if k not in ["deletion_percent", "deletion_mode", "num_results"]
...
def main():
    ...
    print("=" * 80)
    print("Running hi_dypa robustness evaluations locally")
