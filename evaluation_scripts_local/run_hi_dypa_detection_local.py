#!/usr/bin/env python
"""
Convenience driver to run all hi_dypa-detection configurations locally.

This mirrors `slurm_scripts/run_hi_dypa_detection_hpc.sh` but is designed
for a single workstation. It sequentially invokes
`evaluation_scripts/evaluate_hi_dypa_detection.py` for all nine schemes
...
EVAL_SCRIPT = os.path.join(
    REPO_ROOT, "evaluation_scripts", "evaluate_hi_dypa_detection.py"
)
...
def build_common_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run all hi_dypa-detection configurations locally using the "
            "evaluate_hi_dypa_detection.py script."
        )
    )
    parser.add_argument(
        "--python-bin",
    ...
    parser.add_argument(
        ...
        help="Base output directory (default: evaluation/hi_dypa_detection).",
    )
    parser.add_argument(
        "--run-tag",
        default=None,
...
def collect_and_aggregate_summaries(
    ...
    Args:
        output_dir: Base output directory (e.g., "evaluation/hi_dypa_detection")
...
def main():
    ...
    print("=" * 80)
    print("Running hi_dypa detection evaluations locally")
