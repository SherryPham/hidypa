...
def collect_and_aggregate_summaries(
        ...
        try:
            ...
            if metrics_by_variant:
                ...
                for variant_key, variant_metrics in sorted(metrics_by_variant.items()):
                    row = base_info.copy()
                    row["rewrite_ratio"] = variant_metrics.get("rewrite_ratio", "")
                    row["rewrite_mode"] = variant_metrics.get("rewrite_mode", "")
                    ...
                    for metric_name, metric_value in variant_metrics.items():
                        if metric_name not in ["rewrite_ratio", "rewrite_mode", "num_results"]:
    ...
    # Sort by scheme, then group_bits, then user_bits, then rewrite_ratio, then rewrite_mode
    sort_columns = ["scheme"]
    if "group_bits" in df.columns:
        sort_columns.append("group_bits")
    if "user_bits" in df.columns:
        sort_columns.append("user_bits")
    if "rewrite_ratio" in df.columns:
        sort_columns.append("rewrite_ratio")
    if "rewrite_mode" in df.columns:
        sort_columns.append("rewrite_mode")
...
def collect_and_aggregate_summaries_concise(
    ...
    config_groups = {}
    ratio_field = "rewrite_ratio"  # For rewrite attack
                ...
                for variant_key, variant_metrics in metrics_by_variant.items():
                    ...
                    if ratio is not None:
                        ...
                        variant_metrics_clean = {
                            ...
                            if k not in ["rewrite_ratio", "rewrite_mode", "num_results"]
